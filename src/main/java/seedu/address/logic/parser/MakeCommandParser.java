...
public class MakeCommandParser implements Parser<MakeCommand> {
    ...
    /**
     * Parses the given {@code String} of arguments in the context of the MakeCommand
    ...
    public MakeCommand parse(String args) throws ParseException {
        ...
        if (trimmedArgs.isEmpty()) {
            throw new ParseException(
                String.format(MESSAGE_INVALID_COMMAND_FORMAT, MakeCommand.MESSAGE_USAGE));
        }

        String[] fileName = trimmedArgs.split("\\s+");

        return new MakeCommand(Paths.get(fileName[0]));
    }
}
