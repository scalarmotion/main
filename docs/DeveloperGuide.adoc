...
Filtering can be done by passing the relevant `Predicate<ResumeEntry>` into `CategoryManager` through `CategoryManger.setFilter()`. The relevant filtered list can be obtained by subsequently calling `CategoryManager.getList()`.

==== CategoryManager
If there comes a need to set a new source list for the `CategoryManager`, create a new instance with the corresponding `List<ResumeEntry>`.

To use `CategoryManager` to filter out relevant entries, there are a few main functions to keep in mind:
* `setList(List<ResumeEntry>)`: sets the source list of entries to filter
* `getList()`: returns `unmodifiableObservableList<ResumeEntry>` of relevant entries
* `setPredicate(Predicate<ResumeEntry>)`: sets the filtering criteria for the list of entries
* `addPredicate(Predicate<ResumeEntry>, String category)`: returns a predicate that builds onto the given predicate to filter by given category as well
* `getPredicate(String category)`: return a predicate that filters entries by the category

===== Example: Listing entries from a certain category `tag ls ~work`
* A `Predicate<ResumeEntry>` will be generated by the command using `CategoryManager.getPredicate()`
* This predicate is then passed along to `Model.filteredList` for the that the display can be updated
...
==== Predicates
Availble predicates (`Predicate<ResumeEntry>`) for use:

* `ContainsCategoryPredicate(String categoryName)`
...
A `Template` object consists of a list of `TemplateSection`, which consists of:

* The section heading
* A `Predicate<ResumeEntry>` which checks that the entry is of the specified `Category`
* A `Predicate<ResumeEntry>` which checks that the entry matches the specified tags

==== Template file
Templates are stored as text files, with each line in the following format:

    [Category Title]:~[Category Tag]:[Tag]&[Tag] [Tag]...

For example:

    Work Experience:~work:java&recent python&recent datascience

Means: include all entries categorized as `work`, as long as they are tagged with `java` AND `recent`, OR `python` AND `recent`, OR `datascience`.
In other words, the list of tags can be treated as a sum of products form, where a `&` represents AND and space represents OR.
...
.. `Taggable` Interface +
Provides an additional abstraction layer for any class that needS to access methods that `ResumeEntry` overrides to implement `Taggable`. This enforces Interface Segregation Principle in which none related classes have limited knowledge about `ResumeEntry`.
...
... Not all entries have EntryInfo, a <<minorEntry, minor entry>> does not contain title, subtitle or duration. `EntryInfo` helps to differentiate <<minorEntry, minor entry>> from <<majorEntry, major entry>> using `isMinorEntry()`.

.. Encapsulation Using `EntryDescription` +
... As opposed to representing an entry description as a `String` field in `ResumeEntry`, encapsulating it in `EntryDescription` provides another layer of abstraction.
... `EntryDescription` contains `List<String>` that allows for easy modification of a specific segment of the desciption of a particular entry, i.e. allows users to edit a particular line of description of a particular entry.


==== Edit Bullet Description (work in progress)
...
The `Resume` object contains a list of `ResumeSections`,
each comprising a title and various `ResumeEntries` associated with it
(as stipulated by the Template used to generate the Resume). +
This Resume object can then be converted to a String using another class.
While we have chosen to implement conversion into Markdown,
it is possible to extend this functionality by implementing your own Converter class
to convert the object into another format, such as HTML.

==== Markdown Conversion

To transform this abstract representation of the Resume into concrete text,
a `MarkdownConverter` utility class is first used to progressively convert each level of the Resume
(from the titles, information and descriptions of each individual ResumeEntry,
to a ResumeSection of ResumeEntries and finally to the full Resume)
into a String containing its formatted Markdown representation.
A file with the name stipulated by the user is then created and the String is written to it.
