...
Tags and categories are single-word keywords tied to individual entries. Each `Entry` can classified under one `Category`, but can be associated with multiple `Tag`.
...
Filtering can be done by passing the relevant `Predicate<Entry>` into `CategoryManager` through `CategoryManger.setFilter()`. The relevant filtered list can be obtained by subsequently calling `CategoryManager.getList()`.

===== CategoryManager
To use `CategoryManager` to filter out relevant entries, there are 3 main functions to keep in mind:

* `setFilter(Predicate<Entry>)`: sets the filtering criteria for the list of entries
* `setList(List<Entry>)`: sets the source list of entries to filter
* `getList()`: returns `unmodifiableObservableList<Entry>` of relevant entries
